     ...
     <div class="right_data" id="sites">
		...
		<form method="post" id="form_menu" action="admin.php?m=content&a=batch_edit_save" >
			<table class="tables" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				$list = M($table_name)->where(array('biaoqian_id' => $content_id))->order('date desc')->limit($offset,$perpage)->select();
				$total_num = M($table_name)->where(array('biaoqian_id' => $content_id))->order('date desc')->count();
				foreach($list as $k=>$v){
