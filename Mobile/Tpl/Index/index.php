...
<?php }?>
    ...
    <div class="product_i common_i_css">
        <?php $classify=M('classify')->where(array('classify_id'=>207))->find();?>
      <div class="common_i_title">
        <h2><?php echo $classify['classify_name'];?></h2>
        <span></span></div>
      <!--产品分类开始-->
...
<?php }?>
    ...
    <div class="more_i"><a href="<?php echo classify_url($classify['type_id'],$classify['classify_id']);?>">查看更多</a></div>
    <!--产品展示结束--> 
    
    <!--关于我们开始-->
    <div class="about_i common_i_css">
      <div class="common_i_title common_i_title_color5">
          <?php $about=M('classify')->where(array('classify_id'=>206))->find();?>
        <h2><?php echo $about['classify_name'];?></h2>
        <span></span></div>
      <div class="about_i_text">  <?php echo $about['classify_intro'];?> </div>
    </div>
    <div class="more_i"><a href="<?php echo classify_url($about['type_id'],$list['classify_id']);?>">查看更多</a></div>
    <!--关于我们结束-->
    
    <!--新闻中心开始-->
    <div class="yewu_i common_i_css">
      <div class="common_i_title">
<?php $listns=M('classify')->where(array('classify_id'=>208))->order('date asc')->find();?>
        <h2><?php echo $listns['classify_name'];?></h2>
        <span></span></div>
      <div class="yewu_i_list">
        <ul>
            <?php $news=M()->table('index_news n,index_relevance r')->where('r.classify_id =208 and r.content_id=n.news_id')->order('date desc')->limit(4)->select();foreach($news as $k=>$v){?>
...
<?php }?>
    ...
    <div class="common_i_css">
      <div class="common_i_title">
<?php $listus=M('classify')->where(array('classify_id'=>212))->order('date asc')->find();?>
